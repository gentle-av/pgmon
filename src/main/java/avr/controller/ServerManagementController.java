...
public class ServerManagementController {
    ...
    public ResponseEntity<MonitoredServer> getServer(@PathVariable String id) {
        return serverManagementService.getServer(id)
    ...
    public ResponseEntity<MonitoredServer> getServerByName(@PathVariable String name) {
        return serverManagementService.getServerByName(name)
            .map(ResponseEntity::ok)
            .orElse(ResponseEntity.notFound().build());
    }

    @PutMapping("/{id}")
    public ResponseEntity<MonitoredServer> updateServer(@PathVariable String id, @RequestBody UpdateServerRequest request) {
        try {
    ...
    public ResponseEntity<PollingConfiguration> getPollingConfig(@PathVariable String id) {
        return serverManagementService.getPollingConfig(id)
            .map(ResponseEntity::ok)
            .orElse(ResponseEntity.notFound().build());
    }

    @PutMapping("/{id}/polling")
    public ResponseEntity<PollingConfiguration> updatePollingConfig(@PathVariable String id, @RequestBody UpdatePollingRequest request) {
        try {
