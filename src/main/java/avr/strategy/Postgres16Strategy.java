...
public class Postgres16Strategy implements PostgresVersionStrategy {
  ...
  public DatabaseStats getDatabaseStats(JdbcTemplate jdbcTemplate, String dbName) {
    String sql = """
        SELECT
            ...
            round(heap_blks_hit * 100.0 / NULLIF(heap_blks_hit + heap_blks_read, 0), 2) as cache_hit,
  ...
  public List<QueryStats> getSlowestQueries(JdbcTemplate jdbcTemplate, int limit, long thresholdMs) {
    // В PostgreSQL 16 колонки называются total_time, mean_time (без _exec)
    String sql = """
  ...
  public List<IndexInfo> getUnusedIndexes(JdbcTemplate jdbcTemplate) {
    ...
    try {
      ...
      log.warn("Ошибка получения индексов (16): {}", e.getMessage());
      return List.of();
    }
  }
  ...
  public String getCacheHitRatioSql() {
    return "SELECT round(heap_blks_hit * 100.0 / NULLIF(heap_blks_hit + heap_blks_read, 0), 2) FROM pg_stat_database WHERE datname = current_database()";
  }
